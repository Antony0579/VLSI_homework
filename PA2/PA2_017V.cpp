...
//---------------------------------------------------//
   ...
   //  string file_name = "D://VS_C++//pa2_0406//testcase1";
    ...
    for (int i = 1; i <= line_num; i++){
        Pointer = &mylist[linein[i]];   
        //  Pointer = Node[第i行--前面的Node]
        mylist[lineout[i]].front_Ptr.push_back(Pointer);
        //  儲存Pointer在(Node[第i行--後面的Node]的前一個Node指標)
        mylist[lineout[i]].front_count += 1;

        Pointer = &mylist[lineout[i]];   
        //  Pointer = Node[第i行--後面的Node]
        mylist[linein[i]].next_Ptr.push_back(Pointer);
        //  儲存Pointer在(Node[第i行--前面的Node]的後一個Node指標)
        mylist[linein[i]].next_count += 1;
    }
